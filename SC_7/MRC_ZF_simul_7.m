function [USR] =MRC_ZF_simul_7(Mrx_org,Nrx_org,K_sbs,SNR_dB,Mtx,Ntx,Ms,LP,duplex,simul)
...
USR=zeros(1,length(Mrx_org));


for ind_mrx=1:length(Mrx_org)
    ...
    if strcmp('HD',duplex)
        ...
        sigma_ck_sq=0;                                   % jth SBS- kth SBS interference
        sigma_m_sq=0;                                    % MBS self interference
        ...
        sigma_sk_sq= 10^(sigma_sk_sq_dB/10);

        sigma_ck_sq=sigma_sk_sq;     

        sigma_m_sq=sigma_sk_sq;
    end
    ...
    for  expn=1:simul
        ...
        hat_H_ul =sqrt(1/2)*(randn(Mrx,K_sbs*Ntx)+1i*randn(Mrx,K_sbs*Ntx))*(sqrt(hat_Beta_ulk_sq));   %estimated channel matrix%check beta values
        til_H_ul=sqrt(1/2)*(randn(Mrx,K_sbs*Ntx)+1i*randn(Mrx,K_sbs*Ntx))*(sqrt(til_Beta_ulk_sq));     %errored channel
        H_ul=hat_H_ul+til_H_ul;




        hat_H_dl =sqrt(1/2)*(randn(K_sbs*Nrx,Mtx*Ms)+1i*randn(K_sbs*Nrx,Mtx*Ms))*(sqrt(hat_Beta_dlk_sq)); %ws
        ...
        g_ckj_min_k =sqrt(1/2)*(randn(Nrx,(K_sbs-1)*Ntx)+1i*randn(Nrx,(K_sbs-1)*Ntx)).*(sqrt(sigma_ck_sq));%check sigma values
        ...
        for k_ind = 1:K_sbs
            ...
            hat_H_dl_kmt=hat_H_dl_k(:,(k_ind-1)*Ms+1:(k_ind*Ms));

            for ms=1:Ms
                hat_h_dl_km=hat_H_dl_kmt(:,ms);
                ...
                g_sk_n=g_sk(:,(k_ind-1)*Ntx+1:(k_ind*Ntx)); %2nd change due to ntx increase
