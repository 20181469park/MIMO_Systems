function [USR] =MRC_ZF_simul_6(Mrx,Nrx,K_sbs,SNR_dB,Mtx,Ntx,Ms,sigma_sk_sq_dB,sigma_ck_sq_dB,sigma_m_sq_dB,LP,duplex,simul)
...
for ind_lp=1:length(sigma_sk_sq_dB)
    ...
    if strcmp('HD',duplex)
        ...
        sigma_ck_sq=10.^(sigma_ck_sq_dB(ind_lp)/10);      % jth SBS- kth SBS interference
        sigma_m_sq=10.^(sigma_m_sq_dB(ind_lp)/10);        % MBS self interference
